...
- Это не клон GatherOS — от него взят только визуальный стиль (см. `manather_todo_description.md`,
  это старое ТЗ по дизайну, актуален из него только раздел про визуал).
...
- [ ] Canvas-доска внутри проекта (мудборд, как Milanote) — частично есть UI Spaces
...
- Сборка: `xcodebuild -project manather.xcodeproj -scheme manather -destination 'platform=macOS' build`
  из папки `manather/`
- Xcode-проект использует FileSystemSynchronized groups — новые .swift-файлы в
  `manather/manather/` подхватываются автоматически
...
```
manather/manather/
├── manatherApp.swift            — точка входа, ModelContainer
├── ContentView.swift            — корневой вью, тема (ManatherTheme)
├── AssetItem.swift              — SwiftData-модель ассета + AssetType
├── GalleryGridView.swift        — главный экран: тулбар, masonry, фильтры, импорт
...
├── ColorIndex.swift             — 7 цветовых корзин, классификация, индексация

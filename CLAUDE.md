...
### Доски (Space / Board) — мудборд-холст внутри проекта
- Вход: правый клик по карточке проекта (вкладка Projects) → **Boards** →
  `BoardListView` (список досок проекта, создание/переименование/дублирование/удаление).
- У проекта может быть **несколько досок** (модели `Board` и `BoardItem` в
  `BoardModels.swift`; `Board.projectName == AssetItem.spaceName`).
...
├── BoardListView.swift          — список досок проекта + создание/редактирование

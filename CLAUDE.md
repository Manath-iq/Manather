...
- Вкладки (левый сайдбар): **General**, **Libraries**, **AI Providers**,
  **CLI Agents**, **About**.
  - **General** — разделы Content (Import from ~/.claude, Load Demo), Storage
    (Clear Image Cache), Global Screenshot Hotkey. Тумблера темы здесь НЕТ — он
    живёт кнопкой sun/moon рядом с шестерёнкой в тулбаре (не дублировать).
  - **Libraries** (`LibrariesSettingsView`, самодостаточна: свой `@Query<Library>`
    + `modelContext`) — список всех библиотек (активная помечена «ACTIVE» +
    счётчик), на каждой: Switch / Export (.zip) / Rename (инлайн) / Delete (с
    подтверждением; последнюю удалить нельзя). Плюс New Library и Import (ZIP).
    Активная библиотека меняется через общий `@AppStorage("activeLibraryID")`;
    `GalleryGridView.onChange` сбрасывает фильтры/выбор при любом переключении.
...
### MCP-сервер (агенты подключаются к библиотеке)

Приложение умеет **само быть MCP-сервером**: запущенный Manather поднимает
локальный HTTP-эндпоинт `http://127.0.0.1:<порт>/mcp`, к которому подключается
AI-агент (Claude Code, Cursor и т.п.). Агент может **искать** в библиотеке и
**брать** ассеты (картинки/сниппеты/скиллы/MCP-конфиги — для медиа отдаётся
абсолютный путь к файлу, агент читает байты сам), а также **создавать**
библиотеки/коллекции, **добавлять** ассеты и **экспортировать** коллекцию в
context-pack. Всё пишется через тот же `ModelContext`, что и UI, поэтому сетка
обновляется вживую.

Архитектура — два слоя (плюс ядро):
- `MCPLibraryService.swift` — **ядро**: все операции (list/search/get, create
  library/collection, add image/snippet/link/skill/mcp, add-to-collection/tags/
  prompt, export). Чистые методы на `ModelContext`, переиспользуют `AssetIngest`-
  логику (`FileManagerHelper`, `ColorIndexer`, `WebsiteScreenshotManager`) и
  `ContextPackExporter.writePack` (его сделали `internal` ради неинтерактивного
  экспорта без save-панели). Возвращают Codable-DTO.
- `MCPHTTPServer.swift` — **транспорт**: мини HTTP/1.1 на `Network.framework`
  (`NWListener`), биндится **только на 127.0.0.1** (`requiredLocalEndpoint =
  .ipv4(.loopback)`, БЕЗ `on:` — иначе конфликт портов и сокет не поднимется),
  проверяет `Authorization: Bearer <token>`. Без сторонних зависимостей.
- `MCPServer.swift` — **протокол**: JSON-RPC 2.0 + MCP (`initialize`/`tools/list`/
  `tools/call`), 16 инструментов с JSON-схемами, диспетчеризация в ядро. Результат
  инструмента — текстовый блок с JSON. Тут же `MCPCapability` — 5 групп разрешений
  (browse/create/add/edit/export), читаются **вживую** из UserDefaults
  (`mcpCap_<group>`, отсутствие ключа = включено): выключенная группа прячет свои
  инструменты из `tools/list` и отклоняется на вызове.
- `MCPServerManager.swift` — **жизненный цикл** (синглтон `.shared`, как
  `ImageCache.shared`): хранит токен/порт/флаг в UserDefaults, старт/стоп сервера.
  Конфигурируется и стартует в `ContentView.onAppear`
  (`MCPServerManager.shared.configure(container:)` + `startIfEnabled()`).
- UI — вкладка **Settings → MCP Server** (`MCPSettingsView` в `SettingsView.swift`):
  главный тумблер + статус. Когда сервер **включён**, ниже раскрываются (а при
  выключении — прячутся) порт, endpoint, токен, кнопки «Copy Claude Code command» /
  «Copy JSON config» и секция **Permissions** — 5 тумблеров возможностей
  (`mcpCap_*`), применяются сразу.

Безопасность: только loopback, доступ по токену, тумблер по умолчанию **выключен**,
API умеет лишь операции с библиотекой (не произвольный файловый доступ/запуск
команд). Песочница и так выключена, поэтому к этому относимся серьёзно. Сервер
работает только пока приложение запущено. Проверка без полноценного клиента —
`curl -X POST http://127.0.0.1:4319/mcp -H "Authorization: Bearer <token>" -d
'{"jsonrpc":"2.0","id":1,"method":"tools/list"}'` (порт/токен — в Settings → MCP).
...
### После MVP
- [x] Many-to-many: ассет в нескольких коллекциях — **реализовано**. Источник
      правды — `AssetItem.collectionNames: [String]`; `collectionName` оставлен как
      «первичная» коллекция (= первая) для обратной совместимости и архивов.
      Хелперы на модели: `inCollection`, `addToCollection`, `removeFromCollection`,
      `toggleCollection`, `setCollections`, `isUnassigned`. Бэкфилл старых данных —
      `migrateToMultiCollections()` (onAppear, идемпотентно). Инспектор —
      мультивыбор чипами (`ChipAssignSection` теперь на `[String]`), контекст-меню
      «Add to Collection» — тоггл с галочками.
...
- [x] Подключение AI-провайдеров (окно настроек: API-ключи в Keychain, Test
      ...
      фичи к ключам:
- [x] AI-фичи поверх подключённых провайдеров — **реализовано**: «Generate
      variation» (`AIClient.generateVariation`), AI-бриф для экспорта
      (`ExportGoalSheet` → `AIClient.chat`) и **vision auto-tag**
      (`AIClient.suggestTags` смотрит на саму картинку; `InspectorView.autoTag`
      использует его, при отсутствии провайдера — фолбэк на слова из названия/промпта).
- [x] Импорт скиллов/MCP-конфигов из `~/.claude/` — **реализовано**
      (`ClaudeImporter.swift`): сканирует `~/.claude/skills/<slug>/SKILL.md` и
      `mcpServers` из `~/.claude.json` (глобальные + по проектам), добавляет как
      ассеты skill/mcpServer, пропуская уже имеющиеся. Кнопка в Settings → General.
- [x] Экспорт прямо в git-репозиторий — **реализовано** (`GitExporter.swift`):
      тумблер в `ExportGoalSheet` запускает `git init` + первый коммит в папке пака
      (использует git-identity пользователя, иначе нейтральный фолбэк). На будущее: `gh repo create`.
...
│
│   — MCP-сервер (агенты подключаются к библиотеке):
├── MCPLibraryService.swift      — ядро: все операции над библиотекой (на ModelContext)
├── MCPHTTPServer.swift          — мини HTTP/1.1 на 127.0.0.1 (NWListener) + токен
├── MCPServer.swift              — JSON-RPC 2.0 / MCP: initialize, tools/list, tools/call
└── MCPServerManager.swift       — синглтон жизненного цикла (токен/порт/старт/стоп)
```
